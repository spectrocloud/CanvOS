...
K8S_VERSIONS = $(if $(strip $(K8S_VERSION)),\
...
build-provider-images: check-k8s-distribution check-versions $(addprefix build-provider-image-,$(strip $(K8S_VERSIONS)))
	@echo "All provider images built successfully"

build-provider-image-%: check-k8s-distribution
	@echo "building for k8s version - $*"
	@env -u K8S_VERSION docker buildx bake $(PROVIDER_TARGET_TYPE) --set *.args.K8S_VERSION=$*

check-versions:
	@$(if $(strip $(K8S_VERSIONS)),,$(error No versions found for K8S_DISTRIBUTION=$(K8S_DISTRIBUTION)))
