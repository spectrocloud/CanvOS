...
.PHONY: base-image build-iso provider-image kairosify build-provider-images
...
provider-image:
	@if [ "$(IS_UKI)" = "true" ]; then \
        docker buildx bake uki-provider-image; \
    else \
        docker buildx bake provider-image; \
    fi

kairosify:
	docker buildx bake -f docker-bake-kairosify.hcl $(if $(filter true,$(PUSH)),--push) kairosify
