...
# We only create efi, oem, recovery, and content (if present)
# Kairos will create COS_STATE and COS_PERSISTENT partitions during boot
# Partition order: efi, oem, recovery, content (if present), then Kairos adds state and persistent
if [ "$HAS_CONTENT" = "true" ] && [ "$CONTENT_SIZE_BYTES" -gt 0 ]; then
    # Calculate final image size with content
    # Partition order: efi, oem, recovery, content
    CALCULATED_IMG_SIZE_BYTES=$(($COS_GRUB_SIZE + $COS_OEM_SIZE + $COS_RECOVERY_SIZE + $CONTENT_SIZE_BYTES + 1024*1024))
else
    # Calculate final image size without content
...
if [ "$HAS_CONTENT" = "true" ] && [ "$CONTENT_SIZE_BYTES" -gt 0 ]; then
    echo "  Content: $(numfmt --to=iec $CONTENT_SIZE_BYTES) (placed at partition 4, right after recovery)"
fi
...
echo ""
echo "Note: Only creating efi, oem, recovery, and content (if present) partitions."
echo "      Kairos will create COS_STATE and COS_PERSISTENT partitions during boot."
if [ "$HAS_CONTENT" = "true" ] && [ "$CONTENT_SIZE_BYTES" -gt 0 ]; then
    echo "      Content partition is placed at partition 4, right after recovery."
...
# Calculate content partition position (if content exists)
# Content is placed right after recovery (partition 4)
if [ "$HAS_CONTENT" = "true" ] && [ "$CONTENT_SIZE_BYTES" -gt 0 ]; then
    CONTENT_START_BYTES=$COS_RECOVERY_END_BYTES
...
# Create partitions
# Partition order: efi, oem, recovery, content (if present)
# Kairos will create COS_STATE and COS_PERSISTENT partitions during boot
if [ "$HAS_CONTENT" = "true" ] && [ "$CONTENT_SIZE_BYTES" -gt 0 ]; then
    echo "Creating partitions: efi, oem, recovery, content"
    # Create 4 partitions: efi, oem, recovery, content
    parted -s "$FINAL_IMG" -- \
      mklabel gpt \
    ...
    if command -v sgdisk >/dev/null 2>&1; then
        ...
        sgdisk -t 4:8300 "$FINAL_IMG" || echo "Warning: Failed to set partition type GUID for content"
    else
    ...
    echo ""
    echo "Note: Created efi, oem, recovery, and content partitions."
    echo "      Content partition is at partition 4, right after recovery."
    echo "      Kairos will create COS_STATE and COS_PERSISTENT partitions during boot."
else
...
if [ "$HAS_CONTENT" = "true" ] && [ "$CONTENT_SIZE_BYTES" -gt 0 ]; then
    FINAL_CONTENT_DEV="${FINAL_PARTS[3]}"
fi
...
# Note: Kairos will create and format COS_STATE and COS_PERSISTENT partitions during boot
if [ "$HAS_CONTENT" = "true" ] && [ "$CONTENT_SIZE_BYTES" -gt 0 ]; then
...
if command -v blkid >/dev/null 2>&1; then
    ...
    echo ""
    if [ "$HAS_CONTENT" = "true" ] && [ "$CONTENT_SIZE_BYTES" -gt 0 ]; then
        echo "Checking for content partition:"
        blkid "$INPUT_IMG"* 2>/dev/null | grep -i "COS_CONTENT" || echo "COS_CONTENT not found in blkid (may need to be mounted)"
        echo ""
        echo "Note: Content partition created at partition 4."
        echo "      COS_STATE and COS_PERSISTENT will be created by Kairos during boot."
    else
