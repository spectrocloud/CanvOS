...
# Args managed by CanvOS / the Earthfile (Dockerfile ARGs it forwards, plus
# every knob in .arg.template) -- never treated as user-custom. A
# CUSTOM_ARG_<one-of-these> is rejected (guard 2) so it is set via its normal
# .arg key. Keep in sync with .arg.template when new build knobs are added.
CANVOS_RESERVED_ARGS="${CANVOS_RESERVED_ARGS:-AMDGPU_DRIVER_RELEASE AMDGPU_DRIVER_SOURCE AMDGPU_INSTALL_SMI AMDGPU_REBUILD_INITRD ARCH AUTO_ENROLL_SECUREBOOT_KEYS \
BASE BUNDLE_K8S_AND_AGENT_PROVIDER CIS_HARDENING CLUSTERCONFIG CUSTOM_TAG DRBD_VERSION \
EDGE_CUSTOM_CONFIG ENROLL_SPECTRO_EXTENSION_CERT FORCE_INTERACTIVE_INSTALL HTTPS_PROXY HTTP_PROXY IMAGE_REGISTRY \
IMAGE_REPO INCLUDE_MS_SECUREBOOT_KEYS INSTALL_AMD_GPU_DRIVERS INSTALL_NVIDIA_GPU_DRIVERS ISO_NAME IS_UKI \
K8S_DISTRIBUTION MAAS_IMAGE_NAME MY_ORG NO_PROXY NVIDIA_DRIVER_BRANCH NVIDIA_DRIVER_TYPE \
NVIDIA_INSTALL_CONTAINER_TOOLKIT NVIDIA_INSTALL_FABRICMANAGER NVIDIA_REBUILD_INITRD NVIDIA_USE_CUDA_REPO OS_DISTRIBUTION OS_VERSION \
PROXY_CERT_PATH UBUNTU_PRO_ATTACH UKI_BRING_YOUR_OWN_KEYS UPDATE_KERNEL}"
