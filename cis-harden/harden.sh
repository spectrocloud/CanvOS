...
harden_auth() {
	...
	# Configuration lines to add to the top of /etc/pam.d/common-auth
	config_lines="auth required pam_faillock.so preauth silent audit deny=5 unlock_time=900"

	# Backup the original file
	cp /etc/pam.d/common-auth /etc/pam.d/common-auth.bak

	# Add configuration lines to the top of the file
	echo -e "$config_lines\n$(cat /etc/pam.d/common-auth)" > /etc/pam.d/common-auth

	echo "Configuration added to /etc/pam.d/common-auth"

	##############Password reuse policy##################

	#Define the password reuse policy
	reuse_policy="password required pam_pwhistory.so remember=5"

	#Check if the common-password file exists
	if [ -e "/etc/pam.d/common-password" ]; then
        	#Check if password reuse policy already exist
        	if grep -q "$reuse_policy" /etc/pam.d/common-password; then
            		echo "Password reuse policy already exist in /etc/pam.d/common-password"
        	else
        		#Add the reuse policy at the end of file
        		echo "$reuse_policy" >> /etc/pam.d/common-password
        		echo "Added lockout policy for failed login attempts in /etc/pam.d/common-password"
        	fi
	else
        	echo "File /etc/pam.d/common-password not found."
	fi
